...
class ChatMessageFormatPlayer
{
	...
	parseMessageAddressee(msg)
	{
		...
		msg.context = addresseeType.context + "->" + addressee.name;
		print(msg.context);

		// For observers only permit public- and observer-chat and PM to observers
		let supposedToSee = isSender || addresseeType.isAddressee(senderID, addresseeGUID);
		print(supposedToSee);
		if (!supposedToSee){
			//we are not supposed to see it so colour it 
			msg.text = "Hidden" + msg.text;
			setStringTags(msg.text, { "color": "230 230 0" })
		}
